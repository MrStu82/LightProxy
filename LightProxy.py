...
from LightParser.smon_decryptor import decrypt_request, decrypt_response
import json
import logging
import os
import proxy
from LightPlugin import *
import socket
import sys
import argparse
import struct
...
VERSION = "0.100"
GITHUB = 'https://github.com/MrStu82/LightProxy'
logger = logging.getLogger("LightProxy")
...
class LightProxyCallback(object):
    def __init__(self):
        self.request = None
    ...
    def onResponse(self, proxy, response):
        ...
        try:
            ...
            try:
                LightPlugin.call_plugins('process_request', (req_json, resp_json))
...
def get_usage_text():
    ...
    lines = []
    lines.append("#"*40)
    lines.append("# LightParser v{} - Light Proxy # ".format(VERSION))
    lines.append("#"*40)
    lines.append("\tWritten by:\n\t\tKaKaRoTo\n")
    lines.append("\tModified by:\n\t\tMrStu82\n")
    lines.append("\tAuthors:")
    for author in authors_text:
        lines.append("\t\t{}".format(author))

    lines.append("\n\tPlugins:")
    for plugin in LightPlugin.plugins:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='LightParser')
    ...
    if not options.no_gui:
        try:
            ...
            from LightParser.gui import gui
