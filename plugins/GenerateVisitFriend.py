...
def parse_visit_data(data):
    ...
    with open("visit-" + str(wizard_id) + "-monsters.csv", "wb") as visit_file:
        ...
        LightPlugin.call_plugins('process_csv_row', ('visit', 'header', (visit_fieldnames, visit_header)))
        ...
        for monster in monsters:
            ...
            LightPlugin.call_plugins('process_csv_row', ('visit', 'monster', (monster, monster_csv)))
            ...
            for rune in monster_runes:
                ...
                LightPlugin.call_plugins('process_csv_row', ('visit', 'rune', (rune, rune_map)))

                visit_writer.writerow(rune_map)

        visit_footer = []

        LightPlugin.call_plugins('process_csv_row', ('visit', 'footer', visit_footer))

        if len(visit_footer) > 0:
            visit_writer.writerows(visit_footer)

class GenerateVisitFriend(LightPlugin):
